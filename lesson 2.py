...
class Auto:
    ...
    def print_passengers_names(self):
        if self.passengers !=[]:
            ...
            print(f"Немає пасажирів в автомобілі {self.brand}")


Kiril = Humen("Kiril")
Igor = Humen("Igor")
car = Auto("BMW")
car.add_passenger(Kiril, Igor)
...
class Human:
    ...
    def days_indexes(self, day):
        day = f"today {day} життя {self.name}"
        print(f"{day:=^50}","\n")
        human_indexes = self.name + "індекси"
        print(f"{human_indexes:=^50}", "\n")
        print(f"Money - {self.money}")
        print(f"Satiaty - {self.satiety}")
        print(f"Gladness - {self.gladness}")
        home_indexes = "Home indexes"
        print(f"{home_indexes:=^50}", "\n")
        print(f"Food - {self.home.food}")
        print(f"Mess - {self.home.mes}")
    ...
    def live (self, day):
        ...
        if self.gladness <= 20:
            if self.home.mess >15:
                print("Я хочу відпочити,але мені потрібно прибирати будинок")
                self.clean_home()
            else:
